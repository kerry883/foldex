/**
 * manim-agent.ts — Separated AI agent for generating Manim animation code.
 * 
 * This agent is analogous to noteagent.ts but specialized for Manim:
 * - Takes a prompt + optional file context
 * - Returns structured output: code, sceneName, title, description, transcript, tags
 * - Has an error-fixing function for retry loops
 */
...
const manimOutputSchema = z.object({
    ...
    sceneName: z.string().describe("The class name of the main scene (e.g., 'CalculusExplainer')"),
    ...
    transcript: z.string().describe("The full voiceover transcript (all text from voiceover calls combined)"),
    tags: z.array(z.string()).describe("3-6 topic classification tags (e.g., ['math', 'calculus', 'derivatives'])"),
});
...
const fixOutputSchema = z.object({
    ...
    sceneName: z.string().describe("The scene class name (may be same as original)"),
